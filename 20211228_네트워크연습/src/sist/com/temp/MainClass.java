 ...
 *      1) 요청  예) 목록보여준다 , 검색 , 로그인 처리 , 회원가입 , 글쓰기... 
 ...
 *            			  => 접속시에 사용자의 정보를 리턴한다 = (리턴형 : Scoket)
 *                                  ========== 발신자 전화번호 
 *                                             (ip,port) => Scoket
 *               Socket accept()
 *            => Socket을 받아서 저장을 한다 (이유는 : 다시 응답하기 위해서 ip기억한다)
...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 나중에는  >>  Document doc = Jsoup.connection("https://wikibook.co.kr/list/").get()    << 이런식으로 
		} catch (Exception ex) {
