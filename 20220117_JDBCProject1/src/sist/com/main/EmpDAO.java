...
public class EmpDAO {
	...
	/*
	 * INNER JOIN : 가장 많이 사용되는 조인 EQUI_JOIN(연산자 =)
	 * 
	 ...
	 * 
	 * SELECT ~ FROM A,B WHERE sal BETWEEN 값 AND 값
	 * 
	 * SELECT ~ FROM A JOIN B ON sal BETWEEN 값 AND 값
	 * 
	 * OUTER JOIN => NULL이 있는 경우에 데이터 읽기 LEFT OUTER JOIN SELECT ~ FROM A,B WHERE
	 * A.col=B.col(+)
	 * 
	...
	public List<EmpVO> empDeptSalgradeJoinData() {
		...
		try {
			...
			/*
			 * ANSI 조인 "SELECT empno, ename, job, hiredate, sal, dname, loc, grade FROM emp
	...
	// 7. SubQuery
	/*
	 * 네트워크 통신 서버 : 오라클 => 응답 클라이언트 : 자바 => 요청 (JDBC) ------- 속도의 문제 : 오라클연결 (DBCP)
	 * ------- 코딩이 너무 길어진다 => ORM (MyBatis,Hibernate) => Spring+Mybatis 서브쿼리 :
	 * SQL+SQL => SQL을 한개로 만드는 경우 네트워크 => 한번에 모아서 전송 ------ SELECT절 : 컬럼형식 =>
	 * 스칼라서브쿼리 (테이블 여러개에서 데이터 추출) 형식) SELECT (SELECT~ ), (SELECT~ ) FROM table_name
	 * 
	 * FROM절 : 테이블 대신 사용 => 인라인 뷰 => Top-N(rownum) , 페이지 나누기 형식) SELECT ~ FROM
	 * (SELECT~) --------- 실행한 컬럼만 사용이 가능
	 * 
	 * SELECT ~ FROM (SELECT ~ FROM (SELECT ~) WHERE절 : 조건값 대신 사용 형식) 단일행 서브쿼리 :
	 * 서브쿼리의 결과값이 1개인 경우 SELECT ~ FROM table_name WHERE 컬럼명 연산자 (SELECT~~)
	 * ---------------- ---------- 메인쿼리 서브쿼리 다중행 서브쿼리 : 서브쿼리 결과값이 여러개인 경우 SELECT ~
	 * FROM table_name WHERE 컬럼명 IN,ANY,ALL,SOME (SELECT~~) ----------------
	 * ---------- 메인쿼리 서브쿼리
	 * 
	 * IN(10,20,30) => 동시 적용 > ANY(10,20,30) => 10 >10 < ANY(10,20,30) => 30 <30 =
	 * ANY(10,20,30) => IN(10,20,30) > ALL(10,20,30) => 30 >30 < ALL(10,20,30) => 10
	 * <10 --------------------------- MIN/MAX
	 * 
	 * WHERE deptno > (최소값) => 10,20,30) deptno > ANY(SELECT DISTINCT deptno FROM
